...
```text
Sketch uses 1146774 bytes (17%) of program storage space. Maximum is 6553600 bytes.
Global variables use 52160 bytes (15%) of dynamic memory, leaving 275520 bytes for local variables.
```

## Test checklist

- Device shows the TabOS `0.5.14-m5` splash screen on boot.
...
- Auto rotate applies the Tab5 one-step display rotation offset while keeping the anti-flicker timing unchanged.
