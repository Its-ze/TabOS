...
System::System()
    : _meshApp(_mesh),
      ...
      _pingApp("ping", "Ping", "PG", "Ping will use the network services in a later tool milestone."),
      _portCheckApp("port", "Port Check", "PC", "TCP checks arrive with network services."),
      _dnsApp("dns", "DNS", "DN", "DNS lookup arrives with network services."),
      _qrApp("qr", "QR Code", "QR", "QR generation is planned for the utilities layer."),
      _atomApp(_hardware),
      _irApp(_hardware),
      _gpioApp(_hardware),
      _relayApp(_hardware),
      ...
      _packetLogApp(_mesh),
      _developerApp("developer", "Developer", "DV", "Developer tools grow around logs and diagnostics."),
